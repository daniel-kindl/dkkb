...
| Production | Serve the validated site to readers through GitHub Pages | `main` after an approved pull request merge | Yes |
...
| Push to `main` after merge | CI verification and the Pages build job | Automatic production deployment |
| `v*` version tag | CI release-state verification | No Pages deployment |
| Manual CI dispatch | Validate the selected ref without publishing it | No deployment |

The only automatic production promotion is a push to `main`. A branch name does not create an environment. Tags identify release states and are verified by CI, while release publication remains the responsibility of the release workflow in issue #109.
...
5. The Pages workflow builds and validates the `main` commit, uploads its artifact, and deploys that artifact to GitHub Pages.
6. The verification job checks the URL returned by the Pages deployment action.
...
The Pages workflow groups runs by workflow and Git ref and does not cancel an in-progress deployment. This prevents a deployment that already passed its build from being interrupted. Multiple merges to `main` may therefore complete in order, with the latest successful `main` deployment becoming the final public state.
